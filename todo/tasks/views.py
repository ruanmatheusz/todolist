...
def taskList(request):
    ...
    filterBtnD = request.GET.get('filterbtnd')
    filterBtnT = request.GET.get('filterbtnt')
    filterBtnDT = request.GET.get('filterbtndt')
    ...
    if search:
        ...
        tasks = Task.objects.filter(done=filter, user=request.user, updated_at__gt=datetime.datetime.now()-datetime.timedelta(days=30))
